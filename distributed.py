...
def patternMatch(allpatterns, params):
    ...
    def splitcluster(allpatterns, index, minNS):
        ...
        print("INDEX", index)
        # if(isinstance(allpatterns, list)):
        #     allpatterns = pd.DataFrame(allpatterns)
        # sublist = allpatterns[allpatterns==index.columns]
        subindex = allpatterns.index(index)
    ...
    meanpatterns = pd.DataFrame(np.transpose(meanpatterns))
    print("MEAN PATTERNS\n", meanpatterns)
...
def corrToMeanPattern(cluster):
    ...
    for column in cluster:
        corrmat.append(pearsonr(cluster[cluster.columns[2]], meanpat)[0])
    return [item for sublist in corrmat for item in sublist]
...
def stitchTogether(finalresult, result, params, sets):
    ...
    if params.coparams["distributed"] == "genome-wide":
        ...
        df2 = []
        for r in result:
            df1.append(r["anndata"])
        combadata = anndata.concat(df1)
        Amean = pd.concat(df1, axis=1)
        Asd = pd.concat(df2, axis=1)
...
def stitchTogether(finalresult, result, params, sets):
    ...
    if params.coparams["distributed"] == "genome-wide":
        ...
        for r in finalresult:
            df1.append(pd.DataFrame(np.array(r["GapsResult"].Amean)))
            df2.append(pd.DataFrame(np.array(r["GapsResult"].Asd)))
        Amean = pd.concat(df1, axis=1)
        Asd = pd.concat(df2, axis=1)
        setindices = [s for st in sets for s in st]
        if(Amean.shape[0] == len(setindices)):
            ...
            if(indices.sort() == setindices.sort()):
                ...
                Asd.reindex(reorder)
        # copy P matrix.. same for all sets
        Pmean = finalresult[0]["GapsResult"].Pmean
        Psd = finalresult[0]["GapsResult"].Psd
        Amean = pycogaps.Matrix(np.array(Amean))
        Asd = pycogaps.Matrix(np.array(Asd))
    else:
        Pmean = pd.DataFrame()
